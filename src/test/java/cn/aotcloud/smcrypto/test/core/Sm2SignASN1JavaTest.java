...
public class Sm2SignASN1JavaTest {

	private static final Logger logger = LoggerFactory.getLogger(Sm2SignASN1JavaTest.class);
    ...
    public void testText() {
		try {
			...
			logger.info("----------------SignASN1 & VerifyASN1Sign(字符串模式)场景描述:采用16进制公私钥对，对字符串形式的明文进行签名验签----------------");
			logger.info("私钥:"+ prvKeyHex);
    		logger.info("公钥:"+ pubKeyHex);
			logger.info("明文:"+ sourceText);
			String signHex = sm2Utils.signASN1FromText(prvKeyHex, sourceText);
			logger.info("签名:" + signHex);
			boolean verify = sm2Utils.verifySignASN1FromText(pubKeyHex, sourceText, signHex);
    ...
    public void testHex() {
		try {
			Sm2Utils sm2Utils = new Sm2Utils();
			logger.info("----------------SignASN1 & VerifyASN1Sign(16进制串模式)场景描述:采用16进制公私钥对，对16进制形式的明文进行签名验签----------------");
			logger.info("私钥:"+ prvKeyHex);
    		logger.info("公钥:"+ pubKeyHex);
			logger.info("明文:"+ sourceHex);
			String signHex = sm2Utils.signASN1FromHex(prvKeyHex, sourceHex);
			logger.info("签名:" + signHex);
			boolean verify = sm2Utils.verifySignASN1FromHex(pubKeyHex, sourceHex, signHex);
			logger.info("验签:" + (verify ? "成功" : "失败"));
			//断言为真
			assertTrue(verify);
			logger.info("--------------------------------\n");
    ...
    public void testData() {
		try {
			Sm2Utils sm2Utils = new Sm2Utils();
			logger.info("----------------SignASN1 & VerifyASN1Sign(字节码模式)场景描述:采用16进制公私钥对，对字节码形式的明文进行签名验签----------------");
			...
			byte[] signData = sm2Utils.signASN1FromData(prvKeyBytes, sourceBytes);
			logger.info("签名:" + Arrays.toString(signData));
			boolean verify = sm2Utils.verifySignASN1FromData(pubKeyBytes, sourceBytes, signData);
			logger.info("验签:" + (verify ? "成功" : "失败"));
			//断言为真
			assertTrue(verify);
			logger.info("--------------------------------\n");
