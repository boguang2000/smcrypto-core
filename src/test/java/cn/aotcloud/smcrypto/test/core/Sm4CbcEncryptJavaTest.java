...
public class Sm4CbcEncryptJavaTest {

	private static final Logger logger = LoggerFactory.getLogger(Sm4CbcEncryptJavaTest.class);
	...
	private final String ivHex = GenerateSm2KeyPair.ivHex;
	private final byte[] ivBytes = ByteUtils.hexToBytes(ivHex);
	
	@Test
    public void testText_CBC() {
		try {
			logger.info("----------------SM4_CBC(字符串模式)场景描述:采用字符串密码，对字符串形式的明文进行加密解密----------------");
			logger.info("明文:" + sourceText);
			logger.info("密码:" + keyHex);
			logger.info("向量:" + ivHex);
			String sm4EncHex = Sm4Utils.CBC.encryptFromText(sourceText, keyHex, ivHex);
			String sm4DecStr = Sm4Utils.CBC.decryptToText(sm4EncHex, keyHex, ivHex);
			...
			logger.warning("SM4_CBC加密解密InvalidSourceDataException异常");
		} catch (InvalidKeyException e) {
			Assert.fail(e.getMessage());
			logger.warning("SM4_CBC加密解密InvalidKeyException异常");
		} catch (InvalidCryptoDataException e) {
			Assert.fail(e.getMessage());
			logger.warning("SM4_CBC加密解密InvalidCryptoDataException异常");
		}
	}
	
	@Test
    public void testHex_CBC() {
		try {
			logger.info("----------------SM4_CBC(16进制串模式)场景描述:采用16进制串密码，对16进制形式的明文进行加密解密----------------");
			logger.info("明文:" + sourceHex);
			logger.info("密码:" + keyHex);
			logger.info("向量:" + ivHex);
			String sm4EncHex = Sm4Utils.CBC.encryptFromHex(sourceHex, keyHex, ivHex);
			String sm4DecHex = Sm4Utils.CBC.decryptToHex(sm4EncHex, keyHex, ivHex);
			logger.info("加密:" + sm4EncHex);
	...
	@Test
    public void testData_CBC() {
		try {
			logger.info("----------------SM4_CBC(字节码模式)场景描述:采用字节码密码，对字节码形式的明文进行加密解密----------------");
			logger.info("明文:" + Arrays.toString(sourceBytes));
			logger.info("密码:" + Arrays.toString(keyBytes));
			logger.info("向量:" + Arrays.toString(ivBytes));
			byte[] sm4EncBytes = Sm4Utils.CBC.encryptFromData(sourceBytes, keyBytes, ivBytes);
			byte[] sm4DecBytes = Sm4Utils.CBC.decryptToData(sm4EncBytes, keyBytes, ivBytes);
